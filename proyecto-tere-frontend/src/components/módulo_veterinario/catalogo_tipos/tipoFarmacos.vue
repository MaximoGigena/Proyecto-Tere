<template>
  ...
  <div class="max-w-4xl mx-auto p-6">
    ...
    <div class="border p-4 rounded-lg shadow bg-white max-w-8xl mx-auto">
      <div v-if="tiposFarmacos.length" class="flex flex-wrap -m-2">
        <div v-for="tipo in tiposFarmacos" :key="tipo.id" class="w-1/3 p-2">
...
const tiposFarmacos = ref([
  { id: 1, nombre: "Antibióticos", descripcion: "Fármacos para combatir infecciones bacterianas", activo: true },
  { id: 2, nombre: "Analgésicos", descripcion: "Fármacos para aliviar el dolor", activo: true },
  { id: 3, nombre: "Antiinflamatorios", descripcion: "Fármacos para reducir inflamaciones", activo: false },
  { id: 4, nombre: "Vitaminas", descripcion: "Suplementos vitamínicos para soporte nutricional", activo: true }
])
...
const eliminarTipo = tipo => {
  if (confirm(`¿Eliminar el tipo de fármaco "${tipo.nombre}"?`)) {
    tiposFarmacos.value = tiposFarmacos.value.filter(t => t.id !== tipo.id)
  }
}

const guardarTipo = () => {
  if (editando.value) {
    const i = tiposFarmacos.value.findIndex(t => t.id === editando.value)
    if (i !== -1) tiposFarmacos.value[i] = { ...form.value }
  } else {
    tiposFarmacos.value.push({ ...form.value, id: Date.now() })
  }
  cerrarModal()
}
