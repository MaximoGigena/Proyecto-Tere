<template>
  ...
  <div class="max-w-4xl mx-auto p-6">
    ...
    <div class="border p-4 rounded-lg shadow bg-white max-w-8xl mx-auto">
      <div v-if="tiposPaliativos.length" class="flex flex-wrap -m-2">
        <div v-for="tipo in tiposPaliativos" :key="tipo.id" class="w-1/3 p-2">
...
const tiposPaliativos = ref([
  { id: 1, nombre: "Control del dolor", descripcion: "Paliativos para aliviar el dolor en enfermedades avanzadas", activo: true },
  { id: 2, nombre: "Soporte nutricional", descripcion: "Terapias paliativas para mantener nutrición y energía", activo: true },
  { id: 3, nombre: "Cuidados respiratorios", descripcion: "Apoyo paliativo para facilitar la respiración", activo: false },
  { id: 4, nombre: "Cuidados paliativos integrales", descripcion: "Manejo integral de síntomas y confort del paciente", activo: true }
])
...
const eliminarTipo = tipo => {
  if (confirm(`¿Eliminar el tipo de paliativo "${tipo.nombre}"?`)) {
...
const guardarTipo = () => {
  if (editando.value) {
    const i = tiposPaliativos.value.findIndex(t => t.id === editando.value)
    if (i !== -1) tiposPaliativos.value[i] = { ...form.value }
  } else {
    tiposPaliativos.value.push({ ...form.value, id: Date.now() })
  }
  cerrarModal()
}
