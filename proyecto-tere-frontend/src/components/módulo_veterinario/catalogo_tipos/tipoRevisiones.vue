<template>
  ...
  <div class="max-w-4xl mx-auto p-6">
    ...
    <div class="border p-4 rounded-lg shadow bg-white max-w-8xl mx-auto">
      <div v-if="tiposRevisiones.length" class="flex flex-wrap -m-2">
        <div v-for="tipo in tiposRevisiones" :key="tipo.id" class="w-1/3 p-2">
...
const tiposRevisiones = ref([
  { id: 1, nombre: "General", descripcion: "Revisión médica general de rutina", activo: true },
  { id: 2, nombre: "Cardiaca", descripcion: "Chequeo especializado del corazón", activo: true },
  { id: 3, nombre: "Dental", descripcion: "Revisión de dientes y encías", activo: false },
  { id: 4, nombre: "Oftalmológica", descripcion: "Revisión de ojos y visión", activo: true }
])
...
const eliminarTipo = tipo => {
  if (confirm(`¿Eliminar el tipo de revisión médica "${tipo.nombre}"?`)) {
...
const guardarTipo = () => {
  if (editando.value) {
    const i = tiposRevisiones.value.findIndex(t => t.id === editando.value)
    if (i !== -1) tiposRevisiones.value[i] = { ...form.value }
  } else {
    tiposRevisiones.value.push({ ...form.value, id: Date.now() })
  }
  cerrarModal()
}
