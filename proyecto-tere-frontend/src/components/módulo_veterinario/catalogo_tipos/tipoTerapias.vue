<template>
  ...
  <div class="max-w-4xl mx-auto p-6">
    ...
    <div class="border p-4 rounded-lg shadow bg-white max-w-8xl mx-auto">
      <div v-if="tiposTerapias.length" class="flex flex-wrap -m-2">
        <div v-for="tipo in tiposTerapias" :key="tipo.id" class="w-1/3 p-2">
...
const tiposTerapias = ref([
  { id: 1, nombre: "Fisioterapia", descripcion: "Terapia física para recuperación muscular o articular", activo: true },
  { id: 2, nombre: "Terapia Conductual", descripcion: "Terapias para modificación de conducta y hábitos", activo: true },
  { id: 3, nombre: "Hidroterapia", descripcion: "Terapia con agua para rehabilitación", activo: false },
  { id: 4, nombre: "Acupuntura", descripcion: "Terapia alternativa basada en acupuntura", activo: true }
])
...
const eliminarTipo = tipo => {
  if (confirm(`¿Eliminar el tipo de terapia "${tipo.nombre}"?`)) {
...
const guardarTipo = () => {
  if (editando.value) {
    const i = tiposTerapias.value.findIndex(t => t.id === editando.value)
    if (i !== -1) tiposTerapias.value[i] = { ...form.value }
  } else {
    tiposTerapias.value.push({ ...form.value, id: Date.now() })
  }
  cerrarModal()
}
