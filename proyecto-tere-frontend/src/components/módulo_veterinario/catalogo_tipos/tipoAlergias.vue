<template>
  ...
  <div class="max-w-4xl mx-auto p-6">
    ...
    <div class="border p-4 rounded-lg shadow bg-white max-w-8xl mx-auto">
      <div v-if="tiposAlergias.length" class="flex flex-wrap -m-2">
        <div v-for="tipo in tiposAlergias" :key="tipo.id" class="w-1/3 p-2">
...
const tiposAlergias = ref([
  { id: 1, nombre: "Polen", descripcion: "Alergia a pólenes de plantas y árboles", activo: true },
  { id: 2, nombre: "Alimentos", descripcion: "Alergia a ciertos alimentos", activo: true },
  { id: 3, nombre: "Medicamentos", descripcion: "Alergia a fármacos específicos", activo: false },
  { id: 4, nombre: "Ácaros", descripcion: "Alergia a ácaros del polvo", activo: true }
])
...
const eliminarTipo = tipo => {
  if (confirm(`¿Eliminar el tipo de alergia "${tipo.nombre}"?`)) {
    tiposAlergias.value = tiposAlergias.value.filter(t => t.id !== tipo.id)
  }
}

const guardarTipo = () => {
  if (editando.value) {
    const i = tiposAlergias.value.findIndex(t => t.id === editando.value)
    if (i !== -1) tiposAlergias.value[i] = { ...form.value }
  } else {
    tiposAlergias.value.push({ ...form.value, id: Date.now() })
  }
  cerrarModal()
}
