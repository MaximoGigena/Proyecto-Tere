<template>
  ...
  <div class="max-w-4xl mx-auto p-6">
    ...
    <div class="border p-4 rounded-lg shadow bg-white max-w-8xl mx-auto">
      <div v-if="tiposDiagnosticos.length" class="flex flex-wrap -m-2">
        <div v-for="tipo in tiposDiagnosticos" :key="tipo.id" class="w-1/3 p-2">
...
const tiposDiagnosticos = ref([
  { id: 1, nombre: "Infección", descripcion: "Diagnóstico relacionado a infecciones bacterianas o virales", activo: true },
  { id: 2, nombre: "Lesión", descripcion: "Diagnóstico de lesiones físicas o traumatismos", activo: true },
  { id: 3, nombre: "Enfermedad Crónica", descripcion: "Diagnóstico de enfermedades crónicas o degenerativas", activo: false },
  { id: 4, nombre: "Alergia", descripcion: "Diagnóstico de alergias específicas", activo: true }
])
...
const eliminarTipo = tipo => {
  if (confirm(`¿Eliminar el tipo de diagnóstico "${tipo.nombre}"?`)) {
...
const guardarTipo = () => {
  if (editando.value) {
    const i = tiposDiagnosticos.value.findIndex(t => t.id === editando.value)
    if (i !== -1) tiposDiagnosticos.value[i] = { ...form.value }
  } else {
    tiposDiagnosticos.value.push({ ...form.value, id: Date.now() })
  }
  cerrarModal()
}
